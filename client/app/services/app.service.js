...
var ChatService = (function () {
    ...
    ChatService.prototype.getChats = function () {
        if (env_js_1.getEnvVariables().MODE == 'openshift') {
            return this.http.get('http://' + env_js_1.getEnvVariables().APIIP + '/api/chats')
                .map(function (res) { return res.json(); });
        }
        else {
            return this.http.get('http://' + env_js_1.getEnvVariables().APIIP + ':' + env_js_1.getEnvVariables().APIPORT + '/api/chats')
                .map(function (res) { return res.json(); });
        }
    };
    return ChatService;
